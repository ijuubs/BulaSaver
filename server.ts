...
async function startServer() {
  ...
  app.post("/api/concierge", async (req, res) => {
    const { userMessage, history, deals } = req.body;
    
    const simplifiedDeals = deals.slice(0, 100).map((d: any) => ({
      id: d.product_id,
      name: d.name,
...
IMPORTANT: Keep your response concise. Limit your suggested products to a maximum of 10 items to avoid overwhelming the user.`;
    ...
    try {
      ...
      try {
        ...
        res.json(JSON.parse(repairedJson));
      }
    } catch (error) {
      console.error("Concierge Error:", error);
      res.status(500).json({ message: "Concierge is currently unavailable." });
    }
  });
...
`;
    ...
    try {
      ...
      res.status(500).json({ error: "Failed to extract deals." });
    }
  });
